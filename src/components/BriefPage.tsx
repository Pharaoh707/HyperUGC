...
import { Upload, ArrowLeft, Trophy, CheckCircle, HelpCircle } from "lucide-react";
        ...
        <AnimatePresence mode="wait">
          {!isSubmitted ? (
              ...
              <form onSubmit={handleFormSubmit} className="space-y-6 text-left">
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    className={`w-full px-4 py-3 bg-white/70 border border-slate-200 rounded-xl text-slate-800 text-sm focus:outline-none transition-all resize-none ${hoverBorderGlow}`}
                  />
                </div>
            ...
            <motion.div
              ...
              className="glass-panel p-8 sm:p-12 text-center border-emerald-300 shadow-xl space-y-8 bg-white/80"
            >
              <div className="flex justify-center">
                <div className="w-20 h-20 bg-amber-100 border border-amber-300 rounded-full flex items-center justify-center text-amber-500 shadow-inner">
                  <Trophy className="w-10 h-10 animate-bounce" />
                </div>
              </div>

              <div className="space-y-4 max-w-lg mx-auto">
                <div className="flex justify-center items-center space-x-1.5 text-emerald-600 font-bold font-mono text-sm">
                  <CheckCircle className="w-4 h-4" />
                  <span>INTAKE TRANSMITTED SUCCESSFULLY</span>
                </div>
                <h2 className="text-2xl sm:text-3xl font-display font-black text-slate-900 tracking-tight leading-none">
                  Brief received. Let's start building.
                </h2>
                <p className="text-slate-600 text-sm sm:text-base font-semibold leading-relaxed">
                  Your launch rate slot has been frozen. Expect your ad creatives in your inbox within 48–72 hours. Our strategy coordinators will reach out via email if we have any questions.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
                <button
                  onClick={onBack}
                  className="px-6 py-3 bg-gradient-to-b from-slate-700 to-slate-900 border-b-[4px] border-slate-950 text-white rounded-xl text-xs font-bold uppercase tracking-widest shadow-md hover:brightness-110 active:translate-y-[2px] active:border-b-[1px] transition-all cursor-pointer"
                >
                  Return to Landing page
                </button>
                <div className="text-xs text-slate-400 font-medium italic flex items-center space-x-1.5">
                  <HelpCircle className="w-4 h-4" />
                  <span>Support: team@mandjack.digital</span>
                </div>
              </div>
            </motion.div>
          )}
